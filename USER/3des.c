...
/*mode,1表示加密，0表示解密*/
/*length明文的长度*/
int DES3_CRYPT(int mode, unsigned char *key, unsigned char *input, unsigned char *output, int length)
{
	des3_context ctx3;
	int i;//,len=length;
	unsigned char key_buf[16]={0};
	int input_len;
	unsigned char *input_buf,*p;
	input_len = (length%8 == 0 ) ? length:((length/8 +1)*8);
	input_buf = (unsigned char *)malloc(input_len);
	p=input_buf;
	memset(input_buf,0,input_len);
	memcpy(input_buf,input,length);
	memcpy(key_buf,key,16);
	if(mode==DES_ENCRYPT)//加密
	{
		des3_set2key_enc( &ctx3, (unsigned char *) key_buf );
		for(i=0;i<input_len;i+=8)
		{
			des3_crypt_ecb( &ctx3,input_buf, output );
			input_buf+=8;
			output+=8;
		}
	...
	{
		des3_set2key_dec( &ctx3, (unsigned char *) key_buf );
		for(i=0;i<input_len;i+=8)
		{
			des3_crypt_ecb( &ctx3,input_buf, output );
			input_buf+=8;
